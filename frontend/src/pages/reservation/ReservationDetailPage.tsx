...
import { cancelReservation, getReservationById } from "@/features/reservation/api/reservation.api";
...
export default function ReservationDetailPage() {
  ...
  const {
  mutate: cancelReservationMutation,
  isPending: isCancelling,
} = useMutation({
  mutationFn: () => cancelReservation(parsedId!),
  onSuccess: () => {
        ...
        <div className="mt-5 flex items-center justify-between text-base font-bold text-slate-900">
          ...
          {reservation.reservationStatus === "WAITING_PAYMENT" && (
            <div className="flex justify-end gap-3">
              <button
                ...
                className="rounded-lg bg-midnight-indigo px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition-all hover:opacity-90"
              >
                Proceed to Payment
              </button>
              ...
              <button
                ...
                className="rounded-lg border border-rose-200 bg-white px-5 py-2.5 text-sm font-semibold text-rose-600 shadow-sm transition-all hover:bg-rose-50 disabled:cursor-not-allowed disabled:opacity-50"
              >
          ...
          <div className="flex justify-end">
            <button
              type="button"
              onClick={() =>
                navigate(`/reviews/${reservation.id}`)
              }
              className="rounded-lg bg-midnight-indigo px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition-all hover:opacity-90"
            >
              Write a Review
